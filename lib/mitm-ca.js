 ...
 * Generates PEM files under .data/projects/<slug>/mitm/ using Node crypto.
...
function mitmDir(projectSlug) {
  ensureProjectDirs(projectSlug);
  const dir = path.join(projectDataDir(projectSlug), 'mitm');
...
function ensureCa(projectSlug) {
  const dir = mitmDir(projectSlug);
...
function hostCert(projectSlug, hostname) {
  const { keyPath: caKey, certPath: caCert, dir } = ensureCa(projectSlug);
...
function createMitmCa(projectSlug) {
  const tls = require('tls');
  const { certPath } = ensureCa(projectSlug);
  const cache = new Map();
  ...
  return {
    ...
    getSecureContext(hostname) {
      ...
      const pems = hostCert(projectSlug, hostname);
