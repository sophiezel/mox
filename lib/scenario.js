...
function assertScenarioShape(raw, name) {
  ...
  if (raw.requiredStubs != null) {
    if (!Array.isArray(raw.requiredStubs)) {
      throw new Error(`invalid scenario ${name}: requiredStubs must be array`);
    }
    for (let i = 0; i < raw.requiredStubs.length; i++) {
      if (typeof raw.requiredStubs[i] !== 'string' || !raw.requiredStubs[i].trim()) {
        throw new Error(
          `invalid scenario ${name}: requiredStubs[${i}] must be non-empty string`,
        );
      }
    }
  }
