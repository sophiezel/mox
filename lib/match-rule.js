...
/**
 * Whistle-style path prefix: equal, or urlPath under prefix + '/' boundary.
 * `/v1` matches `/v1` and `/v1/x`, not `/v1xxx`. `/` matches all paths.
 * @param {string|undefined|null} pathPrefix
 * @param {string} urlPath
 * @returns {boolean}
 */
function pathPrefixMatches(pathPrefix, urlPath) {
  if (pathPrefix == null || pathPrefix === '') return true;
...
function matchRule(rules, hostname, urlPath, method, ctx = {}) {
  ...
  for (const rule of rules) {
    ...
    const pathOk = pathPrefixMatches(rule.pathPrefix, urlPath);
    const methodOk =
      !rule.methods ||
