...
const { readProjectIndex, loadServiceRules } = require('../catalog-merge');
...
function writeDomainDraft(opts) {
  ...
  if (opts.projectSlug) {
    const idx = readProjectIndex(opts.projectSlug);
    if (idx?.stubs?.length && !stubs.length) {
      stubs = idx.stubs.map((id) => {
        try {
          const p = parseStubId(id);
          return { stubId: id, method: p.method, path: p.path, upstreamId: p.upstreamId };
        } catch {
          return null;
        }
      }).filter(Boolean).filter((s) => s.upstreamId === upstreamId);
    }
  }
