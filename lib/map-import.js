...
function applyMapImport(filePath, opts = {}) {
  ...
  let savedRule = null;
  // Only persist a rules pack when caller passes a non-empty saveAs string.
  // Default is session-only (no rules/map-import.json pollution).
  const saveAs =
    typeof opts.saveAs === 'string' && opts.saveAs.trim()
      ? opts.saveAs.trim()
      : null;
  if (saveAs) {
