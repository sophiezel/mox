...
function buildInitReport({ apiList, gen, roles, projectDir, projectSlug, taskId, existingContracts }) {
  ...
  const md = [
    ...
    '- **Fidelity**: L0 empty / L1 usage-shape placeholder / L2 captured real body / L3 scenario. Upgrade path: capture-merge, import-openapi, or import-doc.',
    ...
    '# optionally fill shapes from OpenAPI or wiki/Markdown:',
    `mox import-openapi --from=openapi.json --name=${projectSlug}`,
    `mox import-doc --file=./api-notes.md --name=${projectSlug}`,
    '```',
    '',
  ].join('\n');
