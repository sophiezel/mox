...
const GAP_TYPES = {
  ...
  dynamic_key: {
    layer: 'trace',
    staticallyResolvable: false,
    disposition: 'capture-merge or import-openapi / import-doc',
  },
};
...
const FIDELITY_LEVELS = {
  L0: {
    ...
    upgradeHint: 'Run `mox capture-merge` after browsing the flow, or `import-openapi` / `import-doc` to supply shape.',
  },
  L1: {
  ...
  L3: {
    ...
    description:
      'Virtual Service (store/protocol) or multi-scenario cases beyond standard success/empty/error.',
    upgradeHint: 'Seed + protocol handlers serve parameterized responses from store.',
  },
};
...
function classifyFidelity(contract) {
  ...
  if (contract.virtualService && contract.virtualService.protocol) {
    return 'L3';
  }
