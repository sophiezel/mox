...
const { isCaptureNoiseHost, normalizeProxyMode } = require('./capture-filter');

/** @typedef {'mock'|'fail'|'capture'|'upstream'|'connect'|'tool'} AccessLabel */
...
const ACTION_CLASS = {
  ...
  capture: { label: 'capture', bucket: 'signal' },
...
function shouldPrintConsole(entry, level) {
  ...
  return (
    bucket === 'signal' &&
    (label === 'mock' || label === 'fail' || label === 'capture')
  );
}
 ...
 * Noise-host / capture-open block-write → quiet under summary.
 * @param {object} entry
 * @returns {object}
 */
function enrichAccessEntry(entry) {
  let { label, bucket } = classifyAccess(entry.action);
  if (entry.action === 'block-write') {
    const noiseHost = isCaptureNoiseHost(entry.host);
    const captureOpen =
      normalizeProxyMode(entry.mode) === 'capture-open';
    if (noiseHost || captureOpen) {
      bucket = 'noise';
    }
  }
...
/**
 * Whether a capture record is worth a console "capture" signal (mergeable body).
 * @param {object} rec
 * @returns {boolean}
 */
function captureHasUsableBody(rec = {}) {
  if (rec.reason === 'block-write') return false;
  if (rec.bodyMeta && rec.bodyMeta.parseOk === false) return false;
  if (rec.responseBody == null || rec.responseBody === '') return false;
  return true;
}
