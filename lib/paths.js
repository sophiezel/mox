...
/** Doc import snapshots (wiki / markdown sources). */
function docsDir() {
  return path.join(getDataRoot(), 'docs');
}
...
/** Captures staging only — does not create contracts/mocks/proxy-rules. */
function ensureCapturesDir(upstreamId) {
  const dir = path.join(serviceDataDir(upstreamId), 'captures');
  fs.mkdirSync(dir, { recursive: true });
  return dir;
}
